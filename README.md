...
This will output two files, `stealthbuster.min.js` and `stealthbuster-clean.min.js`. The
...
```javascript
StealthBuster.collector.enableTraps();
```

Then later on a user action that requires validation:
```javascript
const results = await StealthBuster.collector.collect();
...
// as StealthBuster.detector to return 'human'/'bot' based on the flags.

const output = StealthBuster.detector.detect(results);
...
* The `StealthBuster` library's `detector` can take an arbitrary function via its `StealthBuster.detector.setProofOfWorkFn`
